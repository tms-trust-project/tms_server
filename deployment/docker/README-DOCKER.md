...
## Initial Setup
Set up is handled by the `./docker_setup_tms.sh` script, which is invoked as follows:
   - `./docker_setup_tms.sh <image tag>`

where `<image tag>` is the tag of the *tms_server* image to be run, e.g. `./docker_setup_tms.sh dev`.
